...
$CC $PROJECT_SAN_FLAGS $DEBUG_FLAGS -c "$STANDALONE_FUZZ_MAIN" -o /tmp/standalone_main.o
...
build_harness() {
  ...
  $CC $PROJECT_SAN_FLAGS $DEBUG_FLAGS -fsanitize=fuzzer-no-link $INCS -c "$SRC/mayhem/$name.c" -o "/tmp/$name.o"
  # libFuzzer target (the Mayhem target): object + engine + sanitized libs.
  $CC $PROJECT_SAN_FLAGS $DEBUG_FLAGS $LIB_FUZZING_ENGINE "/tmp/$name.o" $libs -o "/mayhem/$name"
  # Standalone reproducer: same object + run-once driver instead of the engine.
  $CC $PROJECT_SAN_FLAGS $DEBUG_FLAGS "/tmp/$name.o" /tmp/standalone_main.o $libs -o "/mayhem/$name-standalone"
}
